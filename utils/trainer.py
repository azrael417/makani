...
from utils.features import get_auxiliary_channels
    ...
    def __init__(self, params, world_rank, job_type="train"):
        ...
        if hasattr(params, "log_to_wandb") and params.log_to_wandb:
            ...
            if not params.resuming:
                ...
                # paralellism:
                tags = [f'ngpu{comm.get_world_size()}',
                        f'mp{comm.get_size("model")}',
                        f'sp{comm.get_size("spatial")}'
                ]

                # initialize wandb
                self.wandb_run = wandb.init(dir=params.wandb_dir,
                                            job_type=job_type,
                                            config=params,
            ...
            if hasattr(params, "dataset"):
                ...
                try:
                    self.wandb_dataset = wandb.use_artifact(params["dataset"]["name"] + ":latest",
                                                            type="dataset")
                    print(f'Using artifact {params["dataset"]["name"] + ":latest"}')
                except:
                    self.wandb_dataset = wandb.Artifact(name=params["dataset"]["name"],
        ...
        if self.world_rank == 0:
            from networks.model_package import save_model_package
        ...
        if params.scheduler == 'ReduceLROnPlateau':
            ...
            self.scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, factor=params.scheduler_factor,
                                                                        patience=params.scheduler_patience, mode=params.scheduler_mode)
                    ...
                    for idp, predt in enumerate(predictions):
                        ...
                        for pc, tc in zip(pred_comp, targ_comp):
                            ...
                            if not is_same:
                                ...
                                max_lhs = pc.max().item()
                                max_rhs = tc.max().item()
                                print(f"Error, difference detected at AR step {idp}, eval_step {eval_steps}, substep {substeps}. Mean: lhs = {mean_lhs}, rhs = {mean_rhs}, Min: lhs = {min_lhs}, rhs = {min_rhs}, Max: lhs = {max_lhs}, rhs = {max_rhs}")
                                error_count += 1
